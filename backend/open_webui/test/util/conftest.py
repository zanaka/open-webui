...
these fixtures use a file and point the global session factory at it.
"""
...
def accounts(tmp_path_factory) -> Accounts:
    ...
    engine = create_engine(
        f"sqlite:///{tmp_path_factory.mktemp('encrypted') / 'test.db'}"
    )
    session_factory = sessionmaker(bind=engine)

    # Both the session handed to tests and the sessions the key lookups open
    # for themselves have to land on this database.
