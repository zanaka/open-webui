...
async def get_image_base64_from_file_id(id: str, user=None) -> Optional[str]:
    ...
    # Only the owner can be let in: the content on disk is encrypted with
    # the owner's key, so nobody else could decrypt it anyway.
    if user is None:
        return None
    ...
    try:
        decrypted = await asyncio.to_thread(read_decrypted_file, file, user_id=user.id)
        encoded_string = base64.b64encode(decrypted).decode('utf-8')
        content_type = file.meta.get('content_type') if file.meta else None
        if not content_type:
            # The stored path is an opaque encrypted blob, so the original
            # filename is what carries the extension to guess from.
            content_type, _ = mimetypes.guess_type(file.filename)
        if not content_type and ENABLE_IMAGE_CONTENT_TYPE_EXTENSION_FALLBACK:
            content_type = _IMAGE_MIME_FALLBACK.get(Path(file.filename or '').suffix.lower())
        if not content_type:
            return None
        return f'data:{content_type};base64,{encoded_string}'
    except Exception:
